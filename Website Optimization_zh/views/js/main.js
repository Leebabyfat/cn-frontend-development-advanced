...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var classes = document.getElementsByClassName(".randomPizzaContainer");
    for (var i = 0; i < classes.length; i++) {
      var dx = determineDx(classes[i], size);
      var newwidth = (classes[i].offsetWidth + dx) + 'px';
      classes[i].style.width = newwidth;
    }
  }
  /*function changePizzaSizes(size) {
...
function updatePositions() {
  ...
  var Nums = document.body.scrollTop / 1250 ;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(Nums + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var pizzasize = 256;
  var rows = Math.ceil(window.height / pizzasize);
  ...
  for (var i = 0; i < pizzaNum; i++) {
    ...
    elem.basicLeft = (i % cols) * pizzasize;
    elem.style.top = (Math.floor(i / cols) * pizzasize) + 'px';
